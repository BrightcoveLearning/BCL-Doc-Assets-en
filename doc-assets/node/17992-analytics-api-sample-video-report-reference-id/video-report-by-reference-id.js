var BCLS = (function(window, document, Pikaday) {
  ...
  function buildRequest() {
    var account_id = isDefined($accountID.value) ? $accountID.value : accountID,
      reference_ids = isDefined($reference_id.value)
        ? $reference_id.value.split(",")
        : reference_id.split(","),
      searchStr = "",
      i,
      iMax;
    ...
    options.url +=
      "&limit=all&fields=engagement_score,play_rate,video,video_duration,video_engagement_1,video_engagement_100,video_engagement_25,video_engagement_50,video_engagement_75,video_impression,video_name,video_percent_viewed,video_seconds_viewed,video_view,video.reference_id";
    // add ref id filter
